...
export default function Navbar() {
  ...
  const { isAuthenticated, isAdmin, isPatient, isDoctor, logout } = useAuth();
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          {isAuthenticated && isAdmin && (
            <>
              ...
              <Link to="/admin/specializations">
                <button className={`nav-link${pathname === '/admin/specializations' ? ' active' : ''}`}>Specializations</button>
              </Link>
              <Link to="/admin/reports">
                <button className={`nav-link${pathname === '/admin/reports' ? ' active' : ''}`}>Reports</button>
              </Link>
              <Link to="/admin/settings">
                <button className={`nav-link${pathname === '/admin/settings' ? ' active' : ''}`}>Settings</button>
              </Link>
            </>
          )}
          {isAuthenticated && isPatient && (
            <>
              <Link to="/patient">
                <button className={`nav-link${pathname === '/patient' ? ' active' : ''}`}>Dashboard</button>
              </Link>
              <Link to="/patient/lab-tests">
                <button className={`nav-link${pathname === '/patient/lab-tests' ? ' active' : ''}`}>Lab Tests</button>
              </Link>
            </>
          )}
          {isAuthenticated && isDoctor && (
            <>
              <Link to="/doctor">
                <button className={`nav-link${pathname === '/doctor' ? ' active' : ''}`}>Dashboard</button>
              </Link>
            </>
          )}
        </div>
