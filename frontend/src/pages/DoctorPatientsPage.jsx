...
import { patientService, orderService } from '../services/api';
  ...
  const loadPatients = async () => {
    try {
      ...
      (orders || []).forEach((order) => {
        const pid = order.patientId || order.patientID || order.PatientID;
        const date = order.orderDate || order.OrderDate;
        if (!pid || !date) return;
        if (!orderMap[pid] || new Date(date) > new Date(orderMap[pid])) {
          orderMap[pid] = date;
        }
      });
            ...
            <table style={styles.table}>
              <thead>
                <tr style={styles.tableHeader}>
                  ...
                  <th style={styles.tableHead}>Last Order</th>
                ...
                {filteredPatients.map((patient) => {
                  ...
                  <tr key={pid} style={styles.tableRow}>
                    <td style={styles.tableCell}>
                      <div style={styles.patientName}>{patient.fullName || patient.name || 'N/A'}</div>
                    </td>
                    <td style={styles.tableCell}>{patient.email || 'N/A'}</td>
                    <td style={styles.tableCell}>{formatDate(lastOrders[pid])}</td>
                    <td style={styles.tableCell}>
                      <button
