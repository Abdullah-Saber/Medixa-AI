...
export default function HomePage() {
  ...
  return (
    <div style={{ direction: isRTL ? 'rtl' : 'ltr' }}>
      ...
      {/* ════════════════════ WHY MEDIXA-AI ════════════════════ */}
      <section style={{ padding: '80px 24px', background: 'var(--navy2)', borderTop: '1px solid rgba(11,206,170,.08)' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 56 }}>
            <div className="sl-badge" style={{ marginBottom: 14 }}>WHY CHOOSE US</div>
            <h2 style={{ fontFamily: 'Syne, sans-serif', fontSize: 'clamp(28px,3vw,40px)', fontWeight: 700, letterSpacing: '-.5px' }}>
              Why Medixa-AI?
            </h2>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 24 }}>
            {[
              { title: 'AI-Powered Analysis', desc: 'Get intelligent insights on your test results with our advanced AI interpretation system.', icon: '🤖' },
              { title: 'Fast & Accurate Results', desc: 'Receive your lab results within 24 hours with guaranteed accuracy and precision.', icon: '⚡' },
              { title: 'Expert Doctor Consultations', desc: 'Connect with specialized doctors for expert advice on your health concerns.', icon: '👨‍⚕️' },
              { title: 'Secure & Private', desc: 'Your health data is encrypted and protected with industry-standard security measures.', icon: '🔒' },
            ].map((item, i) => (
              <div key={i} className="sl-card" style={{ textAlign: 'center', padding: 32 }}>
                <div style={{ fontSize: 48, marginBottom: 16 }}>{item.icon}</div>
                <h3 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 600, marginBottom: 12, fontSize: 18 }}>{item.title}</h3>
      ...
      <section style={{ padding: '80px 24px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 56 }}>
            <div className="sl-badge" style={{ marginBottom: 14 }}>PROCESS</div>
            <h2 style={{ fontFamily: 'Syne, sans-serif', fontSize: 'clamp(28px,3vw,40px)', fontWeight: 700, letterSpacing: '-.5px' }}>
              How It Works
            </h2>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 32 }}>
            {[
              ...
              { step: '02', title: 'Book Test', desc: 'Choose and schedule your lab tests' },
              { step: '03', title: 'Get Results', desc: 'Receive results within 24 hours' },
              { step: '04', title: 'AI Analysis', desc: 'Get AI-powered health insights' },
            ].map((item, i) => (
              <div key={i} style={{ position: 'relative' }}>
                <div style={{ fontSize: 48, fontWeight: 700, color: 'rgba(11,206,170,.2)', fontFamily: 'Syne, sans-serif', marginBottom: -20, zIndex: 0 }}>{item.step}</div>
                <div style={{ position: 'relative', zIndex: 1, paddingTop: 20 }}>
                  <h3 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 600, marginBottom: 8, fontSize: 18 }}>{item.title}</h3>
      ...
      {/* ════════════════════ AI INTERPRETATION ════════════════════ */}
      <section style={{ padding: '80px 24px', background: 'linear-gradient(135deg, rgba(11,206,170,.08) 0%, rgba(11,206,170,.02) 100%)', borderTop: '1px solid rgba(11,206,170,.08)', borderBottom: '1px solid rgba(11,206,170,.08)' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 60, alignItems: 'center' }}>
          <div>
            <div className="sl-badge" style={{ marginBottom: 14 }}>AI TECHNOLOGY</div>
            <h2 style={{ fontFamily: 'Syne, sans-serif', fontSize: 'clamp(28px,3vw,40px)', fontWeight: 700, marginBottom: 20, letterSpacing: '-.5px' }}>
              AI-Powered Health Insights
            </h2>
            <p style={{ color: 'var(--muted)', fontSize: 15, lineHeight: 1.7, marginBottom: 24 }}>
              Our advanced AI system analyzes your lab test results and provides intelligent interpretations, risk assessments, and personalized recommendations. Understand your health data like never before.
            </p>
            <ul style={{ listStyle: 'none', padding: 0, marginBottom: 32 }}>
              {['Intelligent result interpretation', 'Risk level assessment', 'Personalized health recommendations', 'Trend analysis over time'].map((item, i) => (
                <li key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12, fontSize: 14 }}>
                  <span style={{ color: '#0bceaa' }}>✓</span>
          ...
          <div style={{ background: 'var(--navy2)', border: '1px solid rgba(11,206,170,.2)', borderRadius: 20, padding: 32, position: 'relative' }}>
            <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 16, letterSpacing: 1 }}>AI ANALYSIS EXAMPLE</div>
            <div style={{ background: 'rgba(11,206,170,.05)', borderRadius: 12, padding: 20, marginBottom: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                <span style={{ fontSize: 14, fontWeight: 600 }}>Cholesterol Level</span>
                <span style={{ fontSize: 14, color: '#0bceaa', fontWeight: 600 }}>245 mg/dL</span>
              </div>
              <div style={{ height: 4, background: 'rgba(11,206,170,.2)', borderRadius: 2, marginBottom: 8 }}>
                <div style={{ width: '75%', height: '100%', background: '#f39c12', borderRadius: 2 }}></div>
              </div>
              <div style={{ fontSize: 12, color: 'var(--muted)' }}>Above normal range</div>
            </div>
            <div style={{ fontSize: 13, color: 'var(--text)', lineHeight: 1.6 }}>
              <strong style={{ color: '#0bceaa' }}>AI Insight:</strong> Your cholesterol level is elevated. Consider dietary changes and consult with a cardiologist for personalized advice.
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: '80px 24px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 56 }}>
            <div className="sl-badge" style={{ marginBottom: 14 }}>TESTIMONIALS</div>
            <h2 style={{ fontFamily: 'Syne, sans-serif', fontSize: 'clamp(28px,3vw,40px)', fontWeight: 700, letterSpacing: '-.5px' }}>
              What Our Users Say
            </h2>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 24 }}>
            {[
              { name: 'Sarah Ahmed', role: 'Patient', text: 'Medixa-AI made it so easy to track my health. The AI interpretation feature is incredible!', rating: 5 },
              { name: 'Dr. Mohamed Hassan', role: 'Doctor', text: 'The platform helps me provide better care to my patients with quick access to their results.', rating: 5 },
              { name: 'Omar Khaled', role: 'Patient', text: 'Fast, accurate, and secure. I recommend Medixa-AI to everyone looking for reliable lab services.', rating: 5 },
            ].map((item, i) => (
              <div key={i} className="sl-card" style={{ padding: 24 }}>
                <div style={{ display: 'flex', gap: 4, marginBottom: 16 }}>
                  {[...Array(item.rating)].map((_, j) => (
                    <span key={j} style={{ color: '#f39c12' }}>★</span>
                  ))}
                </div>
                <p style={{ color: 'var(--text)', fontSize: 14, lineHeight: 1.6, marginBottom: 20, fontStyle: 'italic' }}>
                  "{item.text}"
                </p>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'rgba(11,206,170,.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>
                    👤
                  </div>
                  <div>
      ...
      <section style={{ padding: '80px 24px', background: 'var(--navy2)', borderTop: '1px solid rgba(11,206,170,.08)' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 56 }}>
            <div className="sl-badge" style={{ marginBottom: 14 }}>CONTACT US</div>
            <h2 style={{ fontFamily: 'Syne, sans-serif', fontSize: 'clamp(28px,3vw,40px)', fontWeight: 700, letterSpacing: '-.5px' }}>
              Get In Touch
            </h2>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 24 }}>
            {[
              ...
              { icon: '📞', title: 'Phone', value: '+20 123 456 7890' },
              { icon: '✉️', title: 'Email', value: 'support@medixa-ai.com' },
              { icon: '🕐', title: 'Hours', value: 'Mon-Sat: 8AM - 8PM' },
            ].map((item, i) => (
              <div key={i} className="sl-card" style={{ textAlign: 'center', padding: 24 }}>
                <div style={{ fontSize: 32, marginBottom: 12 }}>{item.icon}</div>
                <h3 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 600, marginBottom: 8, fontSize: 16 }}>{item.title}</h3>
                <p style={{ color: 'var(--muted)', fontSize: 14 }}>{item.value}</p>
              </div>
            ))}
          </div>
        </div>
